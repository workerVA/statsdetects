    ...
    for i in readFile:
        ...
        if len(commonTable.loc[commonTable['HASH'] == i.splitlines()[0]]['URL']) > 0:
            for urlForWork in commonTable.loc[commonTable['HASH'] == i.splitlines()[0]]['URL']:
                ...
                os.system("timeout 40m /opt/codeqlmy/codeql/codeql database create --language=cpp --source-root="+folderGitClone+"  -- "+folderPRJ+" &&sudo echo $? > /tmp/echoExitCode")
                ...
                if echoCode.startswith("0"):
                    ...
                    if os.path.exists("/opt/codeqlmy/codeql-repo/cpp/ql/src/experimental/Security/CWE/CWE-XXX/"):
                        shutil.rmtree("/opt/codeqlmy/codeql-repo/cpp/ql/src/experimental/Security/CWE/CWE-XXX/")
                    if not os.path.exists("/opt/codeqlmy/codeql-repo/cpp/ql/src/experimental/Security/CWE/CWE-XXX/"):
                        os.system("mkdir /opt/codeqlmy/codeql-repo/cpp/ql/src/experimental/Security/CWE/CWE-XXX/")
                    os.system("cp "+pathToQuery.splitlines()[0]+" /opt/codeqlmy/codeql-repo/cpp/ql/src/experimental/Security/CWE/CWE-XXX/1.ql")
                    start_time = datetime.now()

                    os.system("timeout 40m /opt/codeqlmy/codeql/codeql query run --database="+folderPRJ+" --output=bqrsprj.bqrs -- /opt/codeqlmy/codeql-repo/cpp/ql/src/experimental/Security/CWE/CWE-XXX/1.ql")
